...
class ActorModel(Model):
    def __init__(self,
        ...
        self.input_fc = layers.Dense(hidden_size, activation='relu', name='actor_input_dense')
        ...
        # 3. Final Dense Layer
        self.fc_out = layers.Dense(output_size, name='actor_output_dense')
    ...
    def call(self, inputs, hidden_states=None, training=None):
        ...
        logits = self.fc_out(output, training=training)
...
class CriticModel(Model):
    def __init__(self,
        ...
        # 1. Initial Dense Layer
        self.input_fc = layers.Dense(hidden_size, activation='relu', name='critic_input_dense')
...
class ActorCriticAgent:
    def __init__(self,
        ...
        self.actor = ActorModel(
            ...
            num_layers=actor_layers, prob_connection=actor_prob_connection, layer_type=layer_type
        )
        self.critic = CriticModel(
            input_size=obs_size, hidden_size=critic_hidden_size, num_layers=critic_layers,
    ...
    def evaluate_state(self, state, critic_hidden_states=None, training=True):
        ...
        state_tensor = tf.expand_dims(state_tensor, axis=0)
        state_tensor = tf.expand_dims(state_tensor, axis=1)
        value, new_critic_hidden_states = self.critic(
            state_tensor, hidden_states=critic_hidden_states, training=training
        )
